...
const dst = path.join(__dirname, 'Application_form', '在留資格認定証明書', '001453107_extracted');

// 1. Read external link XML
const extLinkPath = path.join(dst, 'xl', 'externalLinks', 'externalLink1.xml');
if (!fs.existsSync(extLinkPath)) {
    console.log('externalLink1.xml not found.');
    process.exit(1);
}
...
// Write JSON
const outPath = path.join(__dirname, 'src', 'lib', 'constants', 'dropdowns_coe.json');
