...
const relativeSchema = z.object({
  relationship: requiredString.describe('sheet1:AE-BN-BS 続柄'),
  name: requiredString.describe('sheet1:AF-BO-BT 氏名'),
  birthDate: dateString.describe('sheet1:AG-BP-BU 生年月日'),
  nationality: requiredString.describe('sheet1:AH-BQ-BV 国籍・地域'),
...
export const foreignerInfoSchema = z
  .object({
    ...
    currentResidenceStatus: requiredString.describe('sheet1:S 現に有する在留資格'),
    ...
    desiredResidenceStatus: z.enum(['specified_skilled_worker']).or(z.string()).describe('sheet1:Y 希望する在留資格'),
    ...
    changeReason: requiredString.min(10, '変更の理由を詳しく入力してください（10文字以上）').describe('sheet1:AA 変更の理由'),
    ...
    residenceCardReceiptMethod: z.enum(['window', 'post']).describe('sheet1:BU 在留カードの受領方法'),
