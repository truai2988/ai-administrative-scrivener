...
光の反射（ハレーション）やピンボケで文字が潰れている場合、または全く関係のない画像（風景やイラストなど）である場合は isValid: false とし、再撮影を促す理由を reason に記載してください。
正しく読み取れる場合のみ isValid: true とし、reason は空文字または「問題ありません」としてください。

結果は必ず以下のJSON形式でのみ返してください。他のテキストやマークダウン指定は含めないでください。

{
  "isValid": true または false,
  "reason": "具体的な理由。例：氏名部分に光が反射して読み取れません。など"
}
`;
