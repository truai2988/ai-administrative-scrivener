...
import { z } from 'zod';
import { simultaneousApplicationSchema } from '@/lib/schemas/changeOfStatusApplicationSchema';
...
export const generateChangeOfStatusCsvSimultaneous = (data: SimultaneousApplication | undefined): Uint8Array => {
  const headers = [
...
export const downloadChangeOfStatusCsvSimultaneous = (data: SimultaneousApplication | undefined, filename = '申請情報入力(同時申請)_1.csv') => {
  const uint8Array = generateChangeOfStatusCsvSimultaneous(data);
