...
const relativeSchema = z.object({
  relationship: z.enum(getEnumValues(renewalFormOptions.relationship)).describe('sheet1:AD-BM-BR 続柄'),
  name: requiredString.max(26, '26文字以内で入力してください').describe('sheet1:AE-BN-BS 氏名'),
  birthDate: dateString.describe('sheet1:AF-BO-BT 生年月日'),
  nationality: z.enum(getEnumValues(renewalFormOptions.nationality)).describe('sheet1:AG-BP-BU 国籍・地域'),
...
const skillCertificationSchema = z.object({
  method: z.enum(getEnumValues(renewalFormOptions.skillCertMethod)).describe('技能水準・日本語能力 評価区分'),
...
export const foreignerInfoSchema = z
  .object({
    nationality: z.enum(getEnumValues(renewalFormOptions.nationality)).describe('sheet1:B 国籍・地域'),
    ...
    gender: z.enum(getEnumValues(renewalFormOptions.gender)).describe('sheet1:E 性別'),
    maritalStatus: z.enum(getEnumValues(renewalFormOptions.maritalStatus)).describe('sheet1:F 配偶者の有無'),
    ...
    currentResidenceStatus: z.enum(getEnumValues(renewalFormOptions.residenceStatus)).describe('sheet1:R 現に有する在留資格'),
    ...
    desiredStayPeriod: z.enum(getEnumValues(renewalFormOptions.desiredStayPeriod)).or(z.literal('')).describe('sheet1:Y 希望する在留期間'),
    ...
    specificSkillCategory: z.enum(getEnumValues(renewalFormOptions.specificSkillCategory)).describe('特定技能の区分'),
    ...
    agentDeliveryInfo: z.enum(getEnumValues(renewalFormOptions.agentDeliveryInfo)).optional().describe('代理人等交付情報提供'),
    ...
    residenceCardReceiptMethod: z.enum(getEnumValues(renewalFormOptions.receiptMethod)).describe('sheet1:BT 在留カードの受領方法'),
    applicantResidencePlace: optionalString.describe('sheet1:BU 申請対象者の住居地'),
    receivingOffice: z.enum(getEnumValues(renewalFormOptions.receivingOffice)).describe('sheet1:BV 受領官署'),
...
export const employerInfoSchema = z.object({
  ...
  paymentMethod: z.enum(getEnumValues(renewalFormOptions.paymentMethod)).describe('sheet2:CW 報酬の支払方法'),
...
const simultaneousCommonInfoSchema = z.object({
  ...
  gender: z.enum(getEnumValues(renewalFormOptions.gender)).optional().describe('性別'),
