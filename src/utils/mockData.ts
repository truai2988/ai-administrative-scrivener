...
import { Foreigner, ForeignerStatus, DEFAULT_BRANCH_ID } from '@/types/database';
...
export const dummyForeigners: Foreigner[] = [
    {
      id: 'DEMO-001',
      ...
      expiryDate: addDays(new Date(), 60).toISOString().split('T')[0],
    ...
    },
    {
      ...
      expiryDate: addDays(new Date(), 21).toISOString().split('T')[0],
      passportImageUrl: 'https://via.placeholder.com/150',
      status: 'チェック中',
      company: '未来創生建設',
      aiReview: {
    ...
    },
    {
      ...
      expiryDate: addDays(new Date(), 120).toISOString().split('T')[0],
      passportImageUrl: 'https://via.placeholder.com/150',
      status: '申請済',
      company: '株式会社テクノレイド',
      aiReview: {
...
export const generateMockData = (count: number): Foreigner[] => {
  ...
  for (let i = 0; i < count - dummyForeigners.length; i++) {
  ...
  return [...dummyForeigners, ...randomData];
};
