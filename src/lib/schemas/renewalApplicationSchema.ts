...
const relativeSchema = z.object({
  relationship: z.enum(getEnumValues(renewalFormOptions.relationship), { message: '選択してください' }).describe('sheet1:AD-BM-BR 続柄'),
  name: requiredString.max(26, '26文字以内で入力してください').describe('sheet1:AE-BN-BS 氏名'),
  birthDate: dateString.describe('sheet1:AF-BO-BT 生年月日'),
  nationality: z.enum(getEnumValues(renewalFormOptions.nationality), { message: '選択してください' }).describe('sheet1:AG-BP-BU 国籍・地域'),
  cohabitation: z.boolean().describe('sheet1:AH-BQ-BV 同居の有無'),
  workplace: z.string().max(60, '60文字以内で入力してください').describe('sheet1:AI-BR-BW 勤務先名称・通学先名称'),
...
const jobHistorySchema = z.object({
  ...
  companyName: requiredString.max(60, '60文字以内で入力してください').describe('sheet2:AM-BN 勤務先名称'),
});

// ─── 技能・日本語能力の証明 ─────────────────────────────────────────────────
// CSV仕様: 試験名・受験地は各最大3個まで繰り返し
const skillCertificationSchema = z.object({
  method: z.enum(getEnumValues(renewalFormOptions.skillCertMethod), { message: '選択してください' }).describe('技能水準・日本語能力 評価区分'),
...
const agencyRepSchema = z.object({
  name: z.string().max(26, '26文字以内で入力してください').optional().describe('取次者(オンラインシステム利用者) (1)氏名'),
...
export const foreignerInfoSchema = z
  .object({
    nationality: z.enum(getEnumValues(renewalFormOptions.nationality), { message: '選択してください' }).describe('sheet1:B 国籍・地域'),
    ...
    gender: z.enum(getEnumValues(renewalFormOptions.gender), { message: '選択してください' }).describe('sheet1:E 性別'),
    maritalStatus: z.enum(getEnumValues(renewalFormOptions.maritalStatus), { message: '選択してください' }).describe('sheet1:F 配偶者の有無'),
    occupation: requiredString.max(40, '40文字以内で入力してください').describe('sheet1:G 職業'),
    homeCountryAddress: requiredString.max(80, '80文字以内で入力してください').describe('sheet1:H 本国における居住地'),
    ...
    japanAddressLines: requiredString.max(85, '85文字以内で入力してください').describe('sheet1:L 日本における住居地(町名丁目番地号等)'),
    ...
    email: z.string().email('正しいメールアドレスを入力してください').max(60, '60文字以内で入力してください').describe('sheet1:O メールアドレス'),

    passportNumber: z.string().regex(/^[A-Za-z0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]*$/, '半角英数字記号で入力してください').max(12, '12文字以内で入力してください').describe('sheet1:P 旅券 (1)番号'),
    ...
    currentResidenceStatus: z.enum(getEnumValues(renewalFormOptions.residenceStatus), { message: '選択してください' }).describe('sheet1:R 現に有する在留資格'),
    ...
    desiredStayPeriod: z.enum(getEnumValues(renewalFormOptions.desiredStayPeriod), { message: '選択してください' }).or(z.literal('')).describe('sheet1:Y 希望する在留期間'),
    desiredStayPeriodOther: optionalString,
    renewalReason: requiredString.min(10, '更新の理由を詳しく入力してください（10文字以上）').max(600, '600文字以内で入力してください').describe('sheet1:Z 更新の理由'),

    criminalRecord: z.boolean().describe('sheet1:AA 犯罪を理由とする処分を受けたことの有無'),
    criminalRecordDetail: z.string().max(40, '40文字以内で入力してください').optional().describe('sheet1:AB 処分の内容'),

    specificSkillCategory: z.enum(getEnumValues(renewalFormOptions.specificSkillCategory), { message: '選択してください' }).describe('特定技能の区分'),
    ...
    depositOrganizationName: z.string().max(60, '60文字以内で入力してください').optional().describe('sheet2:AA 徴収・管理機関名'),
    depositAmount: z.number().optional().describe('sheet2:AB 徴収金額'),
    feeCharged: z.boolean().describe('sheet2:AC 外国機関への費用支払合意の有無'),
    foreignOrganizationName: z.string().max(60, '60文字以内で入力してください').optional().describe('sheet2:AD 外国の機関名'),
    ...
    agentDeliveryInfo: z.enum(getEnumValues(renewalFormOptions.agentDeliveryInfo), { message: '選択してください' }).optional().describe('代理人等交付情報提供'),
    ...
    residenceCardReceiptMethod: z.enum(getEnumValues(renewalFormOptions.receiptMethod), { message: '選択してください' }).describe('sheet1:BT 在留カードの受領方法'),
    applicantResidencePlace: optionalString.describe('sheet1:BU 申請対象者の住居地'),
    receivingOffice: z.enum(getEnumValues(renewalFormOptions.receivingOffice), { message: '選択してください' }).describe('sheet1:BV 受領官署'),
...
const dispatchDestinationSchema = z.object({
  name: z.string().max(60, '60文字以内で入力してください').optional().describe('派遣先 氏名又は名称'),
  ...
  addressLines: z.string().max(85, '85文字以内で入力してください').optional().describe('派遣先 町名丁目番地号等'),
  phone: optionalPhone.describe('派遣先 電話番号'),
  representativeName: z.string().max(26, '26文字以内で入力してください').optional().describe('派遣先 代表者の氏名'),
...
const placementAgencySchema = z.object({
  name: z.string().max(60, '60文字以内で入力してください').optional().describe('職業紹介事業者 氏名又は名称'),
...
const intermediaryAgencySchema = z.object({
  name: z.string().max(60, '60文字以内で入力してください').optional().describe('取次機関 氏名又は名称'),
  ...
  addressLines: z.string().max(85, '85文字以内で入力してください').optional().describe('取次機関 町名丁目番地号等'),
...
const supportAgencySchema = z.object({
  name: z.string().max(60, '60文字以内で入力してください').optional().describe('登録支援機関 (1)氏名又は名称'),
  ...
  addressLines: z.string().max(85, '85文字以内で入力してください').optional().describe('登録支援機関 (4)町名丁目番地号等'),
  phone: optionalPhone.describe('登録支援機関 (4)電話番号'),
  representativeName: z.string().max(26, '26文字以内で入力してください').optional().describe('登録支援機関 (5)代表者の氏名'),
  registrationNumber: optionalString.describe('登録支援機関 (6)登録番号'),
  registrationDate: optionalDateString.describe('登録支援機関 (7)登録年月日'),
  supportOfficeName: z.string().max(60, '60文字以内で入力してください').optional().describe('登録支援機関 (8)支援を行う事業所の名称'),
  ...
  officeAddressLines: z.string().max(85, '85文字以内で入力してください').optional().describe('登録支援機関 (9)事業所町名丁目番地号等'),
  supportSupervisorName: z.string().max(26, '26文字以内で入力してください').optional().describe('登録支援機関 (10)支援責任者名'),
  supportOfficerName: z.string().max(26, '26文字以内で入力してください').optional().describe('登録支援機関 (11)支援担当者名'),
...
export const employerInfoSchema = z.object({
  ...
  paymentMethod: z.enum(getEnumValues(renewalFormOptions.paymentMethod), { message: '選択してください' }).describe('sheet2:CW 報酬の支払方法'),
  ...
  // ─ 特定技能所属機関(1) ─
  companyNameJa: requiredString.max(60, '60文字以内で入力してください').describe('sheet2:EI 氏名又は名称'),
  ...
  companyAddressLines: requiredString.max(85, '85文字以内で入力してください').describe('sheet2:EV 所在地(町名丁目番地号等)'),
  ...
  representativeName: requiredString.max(26, '26文字以内で入力してください').describe('sheet2:FA 代表者の氏名'),

  // ─ 特定技能所属機関(10) 勤務事業所 ─
  workplaceName: requiredString.max(60, '60文字以内で入力してください').describe('sheet2:FB 勤務させる事業所名'),
  ...
  workplaceAddressLines: requiredString.max(85, '85文字以内で入力してください').describe('sheet2:FF 所在地(町名丁目番地号等)'),
  ...
  supportPersonnel: z.object({
    supervisorName: requiredString.max(26, '26文字以内で入力してください').describe('(33)支援責任者名'),
    supervisorTitle: requiredString.max(40, '40文字以内で入力してください').describe('(33)所属・役職'),
    isSupervisorFromStaff: z.boolean().optional().describe('(33)役員・職員からの選任'),
    officerName: requiredString.max(26, '26文字以内で入力してください').describe('(34)支援担当者名'),
    officerTitle: requiredString.max(40, '40文字以内で入力してください').describe('(34)所属・役職'),
...
const reEntryPermitSchema = z.object({
  ...
  travelPurposeOther: z.string().max(60, '60文字以内で入力してください').optional().describe('渡航目的その他'),
  // 予定渡航先国（最大2件）
  destinationCountry1: z.string().max(20, '20文字以内で入力してください').optional().describe('予定渡航先国名1'),
  destinationCountry2: z.string().max(20, '20文字以内で入力してください').optional().describe('予定渡航先国名2'),
  // 出国予定
  departureDatePrimary: optionalDateString.describe('出国予定年月日(主)'),
  departureDateSecondary: optionalDateString.describe('出国予定年月日(副)'),
  departurePortPrimary: z.string().max(20, '20文字以内で入力してください').optional().describe('出国予定の日本の(空)港(主)'),
  departurePortSecondary: z.string().max(20, '20文字以内で入力してください').optional().describe('出国予定の日本の(空)港(副)'),
  // 再入国予定
  ...
  reEntryPortSecondary: z.string().max(20, '20文字以内で入力してください').optional().describe('再入国予定の日本の(空)港(副)'),
  // 希望する再入国許可の種別
  desiredPermitType: optionalString.describe('希望する再入国許可'),
  // 犯罪歴
  hasCriminalRecord: z.boolean().optional().describe('犯罪を理由とする処分を受けたことの有無'),
  criminalRecordDetail: z.string().max(120, '120文字以内で入力してください').optional(),
  // 確定前刑事裁判
  hasPendingCriminalCase: z.boolean().optional().describe('確定前の刑事裁判の有無'),
  pendingCriminalCaseDetail: z.string().max(120, '120文字以内で入力してください').optional(),
  // 旅券取得不可の理由
  noPassportReason: z.string().max(120, '120文字以内で入力してください').optional().describe('旅券取得不可の理由'),
  // 代理人・取次者（再入国申請固有）
...
const activityOutsideStatusSchema = z.object({
  // 現在の在留活動の内容
  currentActivityDescription: z.string().max(120, '120文字以内で入力してください').optional().describe('現在の在留活動の内容'),
  // 他に従事しようとする活動(1) 職務内容（最大3件）
  newActivityJob1: z.string().max(40, '40文字以内で入力してください').optional(),
  newActivityJob2: z.string().max(40, '40文字以内で入力してください').optional(),
  newActivityJob3: z.string().max(40, '40文字以内で入力してください').optional(),
  ...
  // 勤務先(1) 名称（最大2件）
  workplaceName1: z.string().max(60, '60文字以内で入力してください').optional(),
  workplaceName2: z.string().max(60, '60文字以内で入力してください').optional(),
  // 勤務先(2) 所在地
  workplaceZipCode: optionalZipCode,
  workplacePrefecture: optionalString,
  workplaceCity: optionalString,
  workplaceAddressLines: z.string().max(85, '85文字以内で入力してください').optional(),
  workplacePhone1: optionalPhone,
  workplacePhone2: optionalPhone,
  // 勤務先(3) 業種（最大2件）
  workplaceIndustry1: z.string().max(40, '40文字以内で入力してください').optional(),
  workplaceIndustry2: z.string().max(40, '40文字以内で入力してください').optional(),
  workplaceIndustryOther: z.string().max(40, '40文字以内で入力してください').optional(),
  // 代理人・取次者
  agent: agentSchema.optional(),
  agencyRep: agencyRepSchema.optional(),
});
...
const authEmploymentCertSchema = z.object({
  // 証明を希望する活動の内容
  certificationActivityDescription: z.string().max(120, '120文字以内で入力してください').optional().describe('証明を希望する活動の内容'),
  // 就労する期間
  employmentPeriodStart: optionalDateString.describe('就労する期間(始期)'),
  employmentPeriodEnd: optionalDateString.describe('就労する期間(終期)'),
  // 使用目的
  purpose: z.string().max(60, '60文字以内で入力してください').optional().describe('使用目的'),
  // 代理人・取次者
  agent: agentSchema.optional(),
  agencyRep: agencyRepSchema.optional(),
});
...
const simultaneousCommonInfoSchema = z.object({
  ...
  nameEn: z.string().max(40, '40文字以内で入力してください').optional().describe('氏名'),
  gender: z.enum(getEnumValues(renewalFormOptions.gender), { message: '選択してください' }).optional().describe('性別'),
  ...
  addressLines: z.string().max(85, '85文字以内で入力してください').optional().describe('住居地(町名丁目番地号等)'),
  phone: optionalPhone.describe('電話番号'),
  mobilePhone: optionalPhone.describe('携帯電話番号'),
  passportNumber: z.string().regex(/^[A-Z0-9]*$/, '半角英数字で入力してください').max(20, '20文字以内で入力してください').optional().or(z.literal('')).describe('旅券番号'),
