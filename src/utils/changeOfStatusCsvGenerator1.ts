...
import { z } from 'zod';
import { foreignerInfoSchema } from '@/lib/schemas/changeOfStatusApplicationSchema';
 ...
 */
export const generateChangeOfStatusCsv1 = (data: ForeignerInfo): Uint8Array => {
  // 1行ヘッダーの正確な定義
  const headers = [
    '身分事項_国籍・地域',
    '身分事項_生年月日',
    ...
    '身分事項_本国における居住地',
    '身分事項_日本における連絡先_郵便番号',
    '身分事項_日本における連絡先_住居地(都道府県)',
    '身分事項_日本における連絡先_住居地(市区町村)',
    '身分事項_日本における連絡先_住居地(町名丁目番地号等)',
    '身分事項_日本における連絡先_電話番号',
 ...
 */
export const downloadChangeOfStatusCsv1 = (data: ForeignerInfo, filename = '申請情報入力(在留資格変更許可申請)_1.csv') => {
  const uint8Array = generateChangeOfStatusCsv1(data);
