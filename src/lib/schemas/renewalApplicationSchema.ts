...
const relativeSchema = z.object({
  relationship: z.enum(getEnumValues(formOptions.relationship)).describe('sheet1:AD-BM-BR 続柄'),
  name: requiredString.describe('sheet1:AE-BN-BS 氏名'),
  birthDate: dateString.describe('sheet1:AF-BO-BT 生年月日'),
  nationality: z.enum(getEnumValues(formOptions.nationality)).describe('sheet1:AG-BP-BU 国籍・地域'),
...
export const foreignerInfoSchema = z
  .object({
    ...
    currentResidenceStatus: z.enum(getEnumValues(formOptions.residenceStatus)).describe('sheet1:R 現に有する在留資格'),
    ...
    receivingOffice: z.enum(getEnumValues(formOptions.receivingOffice)).describe('sheet1:BV 受領官署'),
