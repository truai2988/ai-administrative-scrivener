...
const relativeSchema = z.object({
  relationship: z.enum(getEnumValues(changeFormOptions.relationship), { message: '選択してください' }).describe('sheet1:AE-BN-BS 続柄'),
  name: requiredStringMax(26).describe('sheet1:AF-BO-BT 氏名'),
  birthDate: dateString.describe('sheet1:AG-BP-BU 生年月日'),
  nationality: z.enum(getEnumValues(changeFormOptions.nationality), { message: '選択してください' }).describe('sheet1:AH-BQ-BV 国籍・地域'),
  ...
  residenceCardNumber: z
    ...
    .optional()
    .or(z.literal(''))
    .describe('sheet1:AK-BT-BY 在留カード番号'),
});
...
const skillCertificationSchema = z.object({
  method: z.enum(['exam', 'technical_intern', 'none'], { message: '選択してください' }).describe('技能水準・日本語能力 評価区分'),
...
export const foreignerInfoSchema = z
  .object({
    nationality: z.enum(getEnumValues(changeFormOptions.nationality), { message: '選択してください' }).describe('sheet1:B 国籍・地域'),
    ...
    gender: z.enum(['male', 'female'], { message: '選択してください' }).describe('sheet1:E 性別'),
    birthPlace: requiredString.describe('sheet1:F 出生地'),
    maritalStatus: z.enum(['married', 'unmarried'], { message: '選択してください' }).describe('sheet1:G 配偶者の有無'),
    ...
    mobileNumber: phoneString.optional().or(z.literal('')).describe('sheet1:O 日本における連絡先 携帯電話番号'),
    email: z.string().email('正しいメールアドレスを入力してください').describe('sheet1:P メールアドレス'),

    passportNumber: z.string().regex(/^[A-Za-z0-9]{7,9}$/, '旅券番号は英数字7〜9桁です').describe('sheet1:Q 旅券 (1)番号'),
    passportExpiryDate: futureDateString.describe('sheet1:R 旅券 (2)有効期限'),
    ...
    currentResidenceStatus: z.enum(getEnumValues(changeFormOptions.residenceStatus), { message: '選択してください' }).describe('sheet1:S 現に有する在留資格'),
    ...
    desiredResidenceStatus: z.enum(getEnumValues(changeFormOptions.desiredStatusOfResidence1), { message: '選択してください' }).describe('sheet1:Y 希望する在留資格'),
    ...
    specificSkillCategory: z.enum(['1', '2'], { message: '選択してください' }).describe('特定技能の区分'),
    ...
    hasRelatives: z.boolean().describe('sheet1:AD 在日親族・同居者の有無'),
    ...
    residenceCardReceiptMethod: z.enum(['window', 'post'], { message: '選択してください' }).describe('sheet1:BU 在留カードの受領方法'),
    applicantResidencePlace: optionalString.describe('sheet1:BV 申請対象者の住居地'),
    receivingOffice: optionalString.describe('sheet1:BW 受領官署'),
    notificationEmail: optionalString.describe('sheet1:BX 通知送信用メールアドレス'),
    checkIntent: z.boolean().describe('sheet1:BY 申請意思の確認'),
    freeFormat: optionalString.describe('sheet1:BZ フリー欄'),
  })
  .refine(
...
export const employerInfoSchema = z.object({
  ...
  paymentMethod: z.enum(['cash', 'bank_transfer'], { message: '選択してください' }).describe('sheet2:CW 報酬の支払方法'),
...
const simultaneousCommonInfoSchema = z.object({
  ...
  gender: z.enum(['male', 'female'], { message: '選択してください' }).optional().describe('性別'),
